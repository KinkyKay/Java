...
public class WorldClockControl extends BorderPane {
    // Obtain all available locales and timezone ids
    ...
    public WorldClockControl() {
        ...
        clock.setLocale(availableLocales[cboLocales.getSelectionModel().getSelectedIndex()]);
        clock.setTimeZone(TimeZone.getTimeZone(availableTimeZones[cboTimeZones.getSelectionModel().getSelectedIndex()]));
        ...
        cboLocales.setOnAction(e -> clock.setLocale(availableLocales[cboLocales.getSelectionModel().getSelectedIndex()]));
        cboTimeZones.setOnAction(e -> clock.setTimeZone(TimeZone.getTimeZone(availableTimeZones[cboTimeZones.getSelectionModel().getSelectedIndex()])));
    }

    private void setAvailableLocales() {
        for (int i = 0; i < availableLocales.length; i++)
            cboLocales.getItems().add(availableLocales[i].getDisplayName() + " " + availableLocales[i].toString());
